...
function loadproducts(data) {
  ...
  for (let i = 0; i < dlt.length; i++) {
    dlt[i].addEventListener("click", function () {
      ...
      validate(
        { product_id: pid },
        "../../models/fetch_seller_products.php",
        function (data) {
        ...
        validate(
          ...
          function (data) {
            if (data.status == "updated") {
              ...
              loadtable();
              document.getElementsByClassName("edit_card")[0].style.display =
                "none";
              search.value = "";
            } else {
              loadtable();
...
if (document.getElementById("stockout_button")) {
  document
    .getElementById("stockout_button")
    .addEventListener("click", function () {
      console.log("dd");
      search.value = "";
